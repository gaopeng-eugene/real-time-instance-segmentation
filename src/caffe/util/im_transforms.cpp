...
void BBoxMirror(NormalizedBBox* bbox){
  Datum* mask = bbox->mutable_mask();
  cv::Mat im_mask = DecodeDatumToCVMat(*mask, 0);
  cv::Mat flip_mask;
  cv::flip(im_mask, flip_mask, 1);
  EncodeCVMatToDatum(flip_mask, "jpg", mask);
...
//  LOG(INFO) << "before mask";
  Datum* new_mask = bbox->mutable_mask();
//  LOG(INFO) << "after mask";
//  LOG(INFO) << "mask address " << new_mask;
//  LOG(INFO) << "address before "<<new_mask;
  cv::Mat im_mask = DecodeDatumToCVMat(*new_mask, 0);
//  LOG(INFO) << "address after "<<new_mask;
  const int im_mask_height = std::min(new_height, y_max - y_min + 1);
  const int im_mask_width = std::min(new_width, x_max - x_min + 1);
  cv::Mat out_im_mask;
  cv::resize(im_mask, out_im_mask, cv::Size(im_mask_width, im_mask_height),0,0,cv::INTER_NEAREST);
  EncodeCVMatToDatum(out_im_mask, "jpg", new_mask);
//  LOG(INFO) << out_im_mask.rows;
}
