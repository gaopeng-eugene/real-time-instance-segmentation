...
template <typename Dtype>
void getMaskGroundTruth(const Dtype* mask_gt_data, const Dtype* gt_data, const int num_gt, 
      const int background_label_id_, const bool use_difficult_gt,
      map<int, vector<cv::Mat> >* all_gt_masks, const int max_mask_width, const int max_mask_height){
   all_gt_masks->clear();
   for(int i=0; i<num_gt; i++){
  //    LOG(INFO)<< "width " <<max_mask_width << "height "<<max_mask_height;
      int start_idx = i * 8;
      int mask_start_idx = i * max_mask_width * max_mask_height;
      int item_id = gt_data[start_idx];
      if(item_id == -1){
           continue;
      }
...
//      cv::Mat gt_mask = cv::Mat(max_mask_height, max_mask_width, CV_8UC1, 0.0); 
      cv::Mat gt_mask = cv::Mat(max_mask_height, max_mask_width , CV_8UC1 , cv::Scalar(0));
      int top_index;
//      LOG(INFO) << max_mask_height * max_mask_width;
      for(int h=0; h<max_mask_height; h++){
//          LOG(INFO)<< "heiget index "<<h;
          int mask_index = 0;
          uchar* ptr = gt_mask.ptr<uchar>(h);
          int h_idx = h;
          for(int w=0; w<max_mask_width;w++){
//             LOG(INFO) << "width index  "<<w;
             int w_idx = w;
             int h_idx_real = h_idx;
             int w_idx_real = w_idx;
             top_index = (i*max_mask_height+h_idx_real)*max_mask_width + w_idx_real;
//             LOG(INFO) << mask_index++ ;
             ptr[mask_index++] = static_cast<uchar>(mask_gt_data[top_index]);
          }
      }
 //     LOG(INFO)<< "width " <<max_mask_width << "height "<<max_mask_height;
 //     std::ostringstream stm ;
 ///     stm<<i;
 //     string filename = "/home/gaopeng/Desktop/mask/" + stm.str()  + ".jpg"; 
 //     LOG(INFO) << filename;
 //     cv::imwrite(filename,gt_mask);

      (*all_gt_masks)[item_id].push_back(gt_mask);
//      LOG(INFO) << gt_mask;
   }
}


template void getMaskGroundTruth(const float* mask_gt_data, const float* gt_data, const int num_gt, 
      const int background_label_id_, const bool use_difficult_gt,
      map<int, vector<cv::Mat> >* all_gt_masks, const int max_mask_width, const int max_mask_height);

template void getMaskGroundTruth(const double* mask_gt_data, const double* gt_data, const int num_gt, 
      const int background_label_id_, const bool use_difficult_gt,
      map<int, vector<cv::Mat> >* all_gt_masks, const int max_mask_width, const int max_mask_height);
...
//template <typename Dtype>
//void GetMaskPredictions(const Dtype* mask_data, const int num,
//      const int num_preds_per_class, const int num_loc_classes,
//      const bool share_location, vector<map<int, vector<Blob> > >* mask_preds, const int mask_size);{
...
//template <typename Dtype>
//void GetMaskPredictions(const float* mask_data, const int num,
//      const int num_preds_per_class, const int num_loc_classes,
//      const bool share_location, vector<map<int, vector<Blob> > >* mask_preds, const int mask_size);

//template <typename Dtype>
//void GetMaskPredictions(const double* mask_data, const int num,
//      const int num_preds_per_class, const int num_loc_classes,
//      const bool share_location, vector<map<int, vector<Blob> > >* mask_preds, const int mask_size);
...
template <typename Dtype>
void EncodeMaskPrediction(const vector<LabelBBox>& all_loc_preds,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      Dtype* mask_pred_data, Dtype* mask_gt_data, const Dtype* mask_data, const map<int, vector<cv::Mat> > all_gt_masks, const int mask_size){
      int num = all_match_indices.size();
      int count = 0;
//      LOG(INFO)<<prior_bboxes.size();
      //change this latter
...
//            LOG(INFO)<<"get into mask encode";
            ...
            for(int j=0; j<match_index.size();j++){
               ...
               const int gt_idx = match_index[j];
               const NormalizedBBox& gt_bbox = all_gt_bboxes.find(i)->second[gt_idx];
               const cv::Mat& gt_mask = all_gt_masks.find(i)->second[gt_idx];
               NormalizedBBox interesct;
               IntersectBBox(gt_bbox, prior_bboxes[j], &interesct);
               cv::Mat crop_gt_mask;
               cv::Mat mask_encode;
               const float gt_x_min = gt_bbox.xmin() * img_width;
               const float gt_x_max = gt_bbox.xmax() * img_width;
               const float gt_y_min = gt_bbox.ymin() * img_height;
               const float gt_y_max = gt_bbox.ymax() * img_height;
               const float anchor_x_min = prior_bboxes[j].xmin() * img_width; 
               const float anchor_x_max = prior_bboxes[j].xmax() * img_width;
               const float anchor_y_min = prior_bboxes[j].ymin() * img_height;
               const float anchor_y_max = prior_bboxes[j].ymax() * img_height;
               const float inter_x_min = interesct.xmin() * img_width;
               const float inter_x_max = interesct.xmax() * img_width;
               const float inter_y_min = interesct.ymin() * img_height;
               const float inter_y_max = interesct.ymax() * img_height;
               float w = gt_x_max - gt_x_min + 1;
               float h = gt_y_max - gt_y_min + 1;
               float mask_w = anchor_x_max - anchor_x_min + 1;
               float mask_h = anchor_y_max - anchor_y_min + 1;
               float inter_in_gt_x_min = inter_x_min - gt_x_min;
               float inter_in_gt_y_min = inter_y_min - gt_y_min;
               float inter_in_gt_w = inter_x_max - inter_x_min + 1;
               float inter_in_gt_h = inter_y_max - inter_y_min + 1;
               float inter_in_encode_x_min = inter_x_min - anchor_x_min;
               float inter_in_encode_y_min = inter_y_min - anchor_y_min;
               float inter_in_encode_w = inter_x_max - inter_x_min + 1;
               float inter_in_encode_h = inter_y_max - inter_y_min + 1;
//               LOG(INFO)<< "before or after ";
//               LOG(INFO)<<"anchor_x_min , x_max, y_min, y_max "<<anchor_x_min<<" "<<anchor_x_max<<" "<<anchor_y_min<<" "<<anchor_y_max;
           
               cv::Rect crop_rect(0,0,std::min(static_cast<int>(w),gt_mask.cols), std::min(static_cast<int>(h), gt_mask.rows));
               gt_mask(crop_rect).copyTo((crop_gt_mask));            
               mask_encode = cv::Mat(static_cast<int>(mask_h), 
                                     static_cast<int>(mask_w) , 
                                     CV_8UC1 , cv::Scalar(0));
               
               cv::Rect inter_in_gt(static_cast<int>(inter_in_gt_x_min), static_cast<int>(inter_in_gt_y_min), 
                                    std::min(static_cast<int>(inter_in_gt_w),img_width), 
                                    std::min(static_cast<int>(inter_in_gt_h),img_height));
               cv::Rect inter_in_encode(static_cast<int>(inter_in_encode_x_min), static_cast<int>(inter_in_encode_y_min), 
                                        std::min(static_cast<int>(inter_in_encode_w),img_width), 
                                        std::min(static_cast<int>(inter_in_encode_h),img_height));
               
               crop_gt_mask(inter_in_gt).copyTo(mask_encode(inter_in_encode));
               for(int img_row=0; img_row<mask_encode.rows; img_row++){
                   uchar* ptr = mask_encode.ptr<uchar>(img_row);
                   for(int img_col=0; img_col<mask_encode.cols; img_col++){
                       int value = static_cast<int>(ptr[img_col]);
                       if(value==2){
                           ptr[img_col] = 1;
                       }
                       value = static_cast<int>(ptr[img_col]);
                       if(value==2){
                           LOG(INFO)<<"we get wront label";
                       }
                   }
               }
               cv::Mat out_mask_encode;
               cv::resize(mask_encode, out_mask_encode, cv::Size(mask_size, mask_size), 0, 0, cv::INTER_NEAREST);
               int coord_rows, coord_cols;

//               LOG(INFO)<<"mask size "<<out_mask_encode.rows <<" "<<out_mask_encode.cols;

               for(int z=0; z < mask_size*mask_size; z++){
                  coord_rows = z % mask_size;
                  coord_cols = z / mask_size;
                  mask_gt_data[count*mask_size*mask_size + z] = static_cast<Dtype>( out_mask_encode.at<uchar>(coord_rows, coord_cols));
//                  LOG(INFO)<<"write label" <<mask_gt_data[count*mask_size*mask_size + z];
//                  LOG(INFO)<<"pointer position"<< count*mask_size*mask_size + z;
               }
               const Dtype* pred_data_pointer = mask_data + 
                                           i * mask_size * mask_size * prior_bboxes.size() 
                                          + j * mask_size * mask_size;
//               LOG(INFO)<<"pred_data_pointer "<<(i * mask_size * mask_size * prior_bboxes.size() 
//                                          + j * mask_size * mask_size);
...
//               LOG(INFO)<<prior_bboxes.size();
               ...
               for(int z=0; z < mask_size*mask_size; z++){
                    mask_pred_data[count*mask_size*mask_size + z] = pred_data_pointer[z];
//                  LOG(INFO)<< (count*mask_size*mask_size + z);
//                    LOG(INFO) << pred_data_pointer[z];
//                  LOG(INFO)<<temp;
//                  mask_pred_data[count*mask_size*mask_size + z] =temp;
//                  LOG(INFO)<<"out";
               }
               
               ++count;
         //      LOG(INFO)<<"mask_w "<< static_cast<int>(mask_w) 
       //                            << "mask_h "
     //                              << static_cast<int>(mask_h);
              // LOG(INFO)<<"w "<< static_cast<int>(w) 
            //                  << "h "
         ///                     << static_cast<int>(h);
       //        LOG(INFO)<<"inter_in_gt_w " 
     //                         << static_cast<int>(inter_in_gt_w) 
   //                           << "inter_in_gt_h "
 //                             << static_cast<int>(inter_in_gt_h);

//               std::ostringstream stm_x, stm_y ;
//               stm_x<< mask_encode.rows;
//               stm_y<< mask_encode.cols; 
//               string filename = "/home/gaopeng/Desktop/mask/resize_mask/" + stm_x.str() + stm_y.str() + ".jpg";
//               cv::imwrite(filename, out_mask_encode);                
              
            }
         }
//      LOG(INFO)<<"get out mask encode";
      }
}


template void EncodeMaskPrediction(const vector<LabelBBox>& all_loc_preds,
      const map<int, vector<NormalizedBBox> >& all_gt_bboxes,
      const vector<map<int, vector<int> > >& all_match_indices,
      const vector<NormalizedBBox>& prior_bboxes,
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      float* mask_pred_data, float* mask_gt_data, const float* mask_data, const map<int, vector<cv::Mat> > all_gt_masks, const int mask_size);
...
template void EncodeMaskPrediction(const vector<LabelBBox>& all_loc_preds,
      ...
      const vector<vector<float> >& prior_variances,
      const MultiBoxLossParameter& multibox_loss_param,
      double* mask_pred_data, double* mask_gt_data, const double* mask_data, const map<int, vector<cv::Mat> > all_gt_masks, const int mask_size);
