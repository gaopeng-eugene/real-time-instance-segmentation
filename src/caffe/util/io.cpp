    ...
    case AnnotatedDatum_AnnotationType_BBOX:
      ...
      if (labeltype == "xml") {
        ...
        return ReadJSONToAnnotatedDatum(labelfile, ori_height, ori_width,
                                        name_to_label, anno_datum, encoding);
...
bool ReadJSONToAnnotatedDatum(const string& labelfile, const int img_height,
    const int img_width, const std::map<string, int>& name_to_label,
    AnnotatedDatum* anno_datum, const std::string &encoding) {
  ptree pt;
...
//  LOG(INFO)<<labelfile;
  ...
  BOOST_FOREACH(ptree::value_type& v1, pt.get_child("annotation")) {
    ...
    bool iscrowd = false;
    ptree object = v1.second;
    string mask_name = object.get<string>("mask_dir");
    // Get category_id.
    ...
    bool status = ReadImageToDatum(mask_name,-1,0,0,0,0,0,encoding,mask);
