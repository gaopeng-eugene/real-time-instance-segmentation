...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
    ...
    mask_size_ = multibox_loss_param_.mask_size();
    mask_bottom_vec_.push_back(&mask_pred_);
    mask_bottom_vec_.push_back(&mask_gt_);
    mask_loss_.Reshape(loss_shape);
    mask_top_vec_.push_back(&mask_loss_);
    
    LayerParameter layer_param;
    layer_param.set_name("mask_sigmoid_loss");
    layer_param.set_type("SigmoidCrossEntropyLoss");
    layer_param.add_loss_weight(Dtype(1.));
    vector<int> mask_shape(1,1);
    mask_shape.push_back(1);
    mask_gt_.Reshape(mask_shape);
    mask_pred_.Reshape(mask_shape);
    
    mask_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
    mask_loss_layer_->SetUp(mask_bottom_vec_, mask_top_vec_);
}
...
//  LOG(INFO) << (bottom[3]->height()) ;
//  LOG(INFO) << (bottom[5]->channels()) ;
...
void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* mask_data = bottom[4]->cpu_data();
  const Dtype* mask_gt_data = bottom[5]->cpu_data();
  //Retrieve all mask ground truth.
  
  
  int max_mask_width = bottom[5]->width();
  int max_mask_height = bottom[5]->height();
//  LOG(INFO)<< max_mask_width;
//  LOG(INFO)<< max_mask_height;

  map<int, vector<cv::Mat> > all_gt_maskes;
  getMaskGroundTruth(mask_gt_data, gt_data, num_gt_, 
      background_label_id_, use_difficult_gt_,
      &all_gt_maskes, max_mask_width, max_mask_height);  
...
//  vector<map<int, vector< Blob<Dtype> > > > all_mask_preds;
...
//  LOG(INFO)<< num_negs ; 
  ...
  if(num_matches_ >=1){
    ...
    mask_shape[0] = 1;
    mask_shape[1] = num_matches_ * mask_size_ * mask_size_;
//    LOG(INFO)<<"entropu loss llength" << mask_shape[1];
    mask_pred_.Reshape(mask_shape);
    mask_gt_.Reshape(mask_shape);
    Dtype* mask_pred_data = mask_pred_.mutable_cpu_data();
    Dtype* mask_gt_data = mask_gt_.mutable_cpu_data();
    caffe_set(num_matches_ * mask_size_ * mask_size_, Dtype(0), mask_pred_data);
    caffe_set(num_matches_ * mask_size_ * mask_size_, Dtype(0), mask_gt_data);
    EncodeMaskPrediction(all_loc_preds, all_gt_bboxes, all_match_indices_,
           prior_bboxes, prior_variances, multibox_loss_param_,
           mask_pred_data, mask_gt_data, mask_data, all_gt_maskes, mask_size_);
//    for(int i=0; i<100; i++){
//       LOG(INFO)<<mask_gt_data[i];
//    }
//    EncodeMaskPrediction(all_loc_preds, all_gt_bboxes, all_match_indices_,
//           prior_bboxes, prior_variances, multibox_loss_param_,
//           mask_pred_data, mask_gt_data);
    mask_loss_layer_->Reshape(mask_bottom_vec_, mask_top_vec_);
    mask_loss_layer_->Forward(mask_bottom_vec_, mask_top_vec_);
  } else {
    mask_loss_.mutable_cpu_data()[0] = 0;
  }
//  LOG(INFO) << "mask loss "<< mask_loss_.mutable_cpu_data()[0];
  ...
  if (this->layer_param_.propagate_down(0)) {
    ...
    LOG(INFO) << "regression loss" << top[0]->mutable_cpu_data()[0];
  }
  if (this->layer_param_.propagate_down(1)) {
    Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
        normalization_, num_, num_priors_, num_matches_);
    top[0]->mutable_cpu_data()[0] += conf_loss_.cpu_data()[0] / normalizer;
    LOG(INFO) << "label classification loss" << conf_loss_.cpu_data()[0] / normalizer;
  }
  
  if (this->layer_param_.propagate_down(4)){
    Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
        normalization_, num_, num_priors_, num_matches_*mask_size_*mask_size_);
    top[0]->mutable_cpu_data()[0] += mask_loss_.cpu_data()[0] / normalizer;
    LOG(INFO) << "label mask prediction loss "<< mask_loss_.cpu_data()[0] / normalizer;
  }
}
...
void MultiBoxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if(propagate_down[4]){
     Dtype* mask_bottom_diff = bottom[4]->mutable_cpu_diff();
     caffe_set(bottom[4]->count(), Dtype(0), mask_bottom_diff);
     if(num_matches_ >= 1){
        vector<bool> mask_propagate_down;
        mask_propagate_down.push_back(true);
        mask_propagate_down.push_back(false);
        mask_loss_layer_->Backward(mask_top_vec_, mask_propagate_down, mask_bottom_vec_);
        Dtype normalizer = LossLayer<Dtype>::GetNormalizer(normalization_, num_, num_priors_, num_matches_*mask_size_*mask_size_);
        Dtype loss_weight = top[0]->cpu_diff()[0] / normalizer;
        caffe_scal(mask_pred_.count(), loss_weight, mask_pred_.mutable_cpu_diff());
        const Dtype*  mask_pred_diff = mask_pred_.cpu_diff();
        int count = 0;
//        LOG(INFO)<< "top[0]->cpu_diff()[0] "<< top[0]->cpu_diff()[0];
//        LOG(INFO)<< "top[0]->cpu_data()[0] "<< top[0]->cpu_data()[0];
//        for(int test=0; test< 10; test++){
//            LOG(INFO)<<"mask diff "<<mask_pred_diff[test];
//        }
        for(int i=0; i<num_; ++i){
            for(map<int, vector<int> >::iterator it=all_match_indices_[i].begin();
                ...
                for(int j=0; j<match_index.size(); ++j){
                    ...
                    int start_idx = mask_size_ * mask_size_ * j;
                    caffe_copy(mask_size_ * mask_size_, 
                               mask_pred_diff+count*mask_size_*mask_size_, 
                               mask_bottom_diff+start_idx); 
                    ++count;
                }
            }
            mask_bottom_diff+=bottom[4]->offset(1);
        }
     }
  }
