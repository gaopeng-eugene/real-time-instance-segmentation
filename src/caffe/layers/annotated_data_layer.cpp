...
void AnnotatedDataLayer<Dtype>::DataLayerSetUp(
  ...
  LOG(INFO)<<"prefetch count"<<this->PREFETCH_COUNT;
  ...
  if (this->output_labels_) {
    ...
    vector<int> label_mask_shape(4,1);
    if (has_anno_type_) {
      ...
      if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX) {
        ...
        label_shape[3] = 8;
        label_mask_shape[0] = 1;
        label_mask_shape[1] = std::max(num_bboxes, 1);
        label_mask_shape[2] = 10;
        label_mask_shape[3] = 10;
    ...
    top[2]->Reshape(label_mask_shape);
    LOG(INFO) <<"annotated layer finished";
    
    for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
      this->prefetch_[i].label_.Reshape(label_shape);
      this->prefetch_[i].label_mask_.Reshape(label_mask_shape);
    }
  }
}
  ...
  if (this->output_labels_ && has_anno_type_) {
    ...
    if (anno_type_ == AnnotatedDatum_AnnotationType_BBOX) {
      ...
      if (num_bboxes == 0) {
        ...
        batch->label_mask_.Reshape(label_shape);
        caffe_set<Dtype>(8, -1, batch->label_mask_.mutable_cpu_data());
      } else {
        ...
        for (int item_id = 0; item_id < batch_size; ++item_id) {
          ...
          for (int g = 0; g < anno_vec.size(); ++g) {
            ...
            for (int a = 0; a < anno_group.annotation_size(); ++a) {
              ...
              width.push_back(bbox.mask().width());
              height.push_back(bbox.mask().height());
            }
          }
        }
        vector<int>::iterator biggest_width = max_element(width.begin(),width.end());
        vector<int>::iterator biggest_height = max_element(height.begin(),height.end());
//        LOG(INFO) <<"get_into";
        label_mask_shape[0] = 1;
        label_mask_shape[1] = num_bboxes;
//        LOG(INFO) << "biggest_width"<< *biggest_height  <<"biggest_height"<< *biggest_width;
        label_mask_shape[2] = *biggest_height;
        label_mask_shape[3] = *biggest_width;
        batch->label_mask_.Reshape(label_mask_shape);
        caffe_set<Dtype>(1*num_bboxes*(*biggest_height)*(*biggest_width), 0, batch->label_mask_.mutable_cpu_data());
        this->data_transformer_->MaskToDatum(all_anno,&(batch->label_mask_));
      }
//    LOG(INFO)<<"bbox number " << num_bboxes; 
//    label_mask_shape[1] = 1;
//    label_mask_shape[2] = 1;
//    label_mask_shape[3] = 1;
//    batch->label_mask_.Reshape(label_mask_shape);
//    this->data_transformer_->MaskToDatum(all_anno,&(batch->label_mask_));
//    LOG(INFO) << "get_out";
//    batch->label_mask_.Reshape(label_shape);
