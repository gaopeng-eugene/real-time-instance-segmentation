  ...
  if (anno_datum.type() == AnnotatedDatum_AnnotationType_BBOX) {
    ...
    for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
      ...
      for (int a = 0; a < anno_group.annotation_size(); ++a) {
        ...
        NormalizedBBox proj_bbox = resize_bbox;
...
template<typename Dtype>
void DataTransformer<Dtype>::MaskToDatum(map<int , vector<AnnotationGroup> >& all_anno,
                                         Blob<Dtype>* label_mask){
  const int channels = label_mask->channels();
  const int height = label_mask->height();
  const int width = label_mask->width();
  const int num = label_mask->num();
  Dtype* mask_blob = label_mask->mutable_cpu_data();
  int mask_index = 0;
  int top_index;
//  LOG(INFO) << "channels" << channels << "  height" <<height <<"  width" <<width<<"  num"<<num;
  for(int item_id=0; item_id < all_anno.size(); ++item_id){
//       LOG(INFO) << "item number " << item_id;
     ...
     for(int g=0; g < anno_vec.size() ; ++g){
        const AnnotationGroup anno_group = anno_vec[g];
            for(int a=0; a<anno_group.annotation_size(); ++a ){
                    const Annotation& anno = anno_group.annotation(a);
                    const NormalizedBBox & bbox = anno.bbox();
                    const Datum & mask = bbox.mask();
                    cv::Mat mask_img;
                    mask_img = DecodeDatumToCVMat(mask, 0);
                    const int img_width = mask_img.cols; 
                    const int img_height =  mask_img.rows;
                    for(int h=0; h<img_height; ++h){
                       int img_index=0;
                       const uchar* ptr = mask_img.ptr<uchar>(h);
                       for(int w=0; w<img_width; ++w){
                          top_index = (mask_index*height + h) * width + w;
                          Dtype pixel = static_cast<Dtype>(ptr[img_index++]);
                          mask_blob[top_index] = pixel;
                       }                       
                    }
                    mask_index++;
//                    LOG(INFO) << "mask height "<<mask_img.channels(); 
            }
     }
  }
//    for(map<int, vector<AnnotationGroup> >::iterator it=all_anno.begin(); it!=all_anno.end(); ++it){
//       LOG(INFO) << "map first element "<< it->first; 
//    }
