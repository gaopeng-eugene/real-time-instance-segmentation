...
def CreateMultiBoxHead(net, data_layer="data", num_classes=[], from_layers=[],
        ...
        conf_postfix='', loc_postfix='', mask_size=21, use_mask=1, **bn_param):
    ...
    for i in range(0, num):
        ...
        if use_mask:
           name = "{}_mbox_mask{}".format(from_layer, conf_postfix)
           num_mask_output = mask_size * mask_size
           ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
               num_output=num_priors_per_location * num_mask_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
           permute_name = "{}_perm".format(name)
           net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
           flatten_name = "{}_flat".format(name)
           net[flatten_name] = L.Flatten(net[permute_name], axis=1)
           mask_layers.append(net[flatten_name])
    ...
    if use_mask:
       name = "mbox_mask"
       net[name] = L.Concat(*mask_layers, axis=1)
       mbox_layers.append(net[name])
